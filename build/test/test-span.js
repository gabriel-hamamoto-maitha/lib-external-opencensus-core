...
describe('Span', () => {
    ...
    describe('get time properties before start()', () => {
        ...
        before(() => {
            const rootSpan = new root_span_1.RootSpan(tracer, name, kind, traceId, parentSpanId);
            rootSpan.start();
            span = new span_1.Span(tracer, rootSpan);
        });
    ...
    describe('start()', () => {
        ...
        it('should start a RootSpan and set CurrentRootSpan when parentSpanId is empty', () => {
            const rootSpan = new root_span_1.RootSpan(tracer, name, kind, traceId, '');
            rootSpan.start();
        ...
        it('should start a RootSpan and set CurrentRootSpan when parentSpanId is not empty', () => {
            const rootSpan = new root_span_1.RootSpan(tracer, name, kind, traceId, 'd5955a12632d46a1');
            rootSpan.start();
            assert.strictEqual(tracer.currentRootSpan, rootSpan);
            const span = new span_1.Span(tracer, rootSpan);
            span.start();
    ...
    describe('addAtribute()', () => {
        it('should add an attribute', () => {
            ...
            ['String', 'Number', 'Boolean'].map(attType => {
                ...
                assert.strictEqual(span.attributes['testKey' + attType], 'testValue' + attType);
            });
    ...
    describe('addLink()', () => {
        it('should add a Link', () => {
            ...
            const span = new span_1.Span(tracer, rootSpan);
            span.start();
            span.addLink(span.traceId, rootSpan.id, types.LinkType.PARENT_LINKED_SPAN);
    ...
    describe('addMessageEvent()', () => {
        it('should add a Message Event', () => {
            ...
            const span = new span_1.Span(tracer, rootSpan);
            span.start();
            span.addMessageEvent(types.MessageEventType.UNSPECIFIED, 
            /* id */ 1, 
    ...
    describe('get traceState()', () => {
        it('should return the traceState', () => {
            const rootSpan = new root_span_1.RootSpan(tracer, name, kind, traceId, parentSpanId, 'traceState');
            rootSpan.start();
